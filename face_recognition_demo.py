...
fd_model = "Models/face-detection-retail-0005.xml"
...
class FrameProcessor:
    ...
    def __init__(self, args):
        ...
        face_detector_net = self.load_model(fd_model)
        ...
        landmarks_net = self.load_model(ld_model)
        #face_reid_net = self.load_model(args.m_reid)
        face_reid_net = self.load_model(id_model)
        ...
        #log.info("Building faces database using images from '%s'" % (face_gallery))
        ...
        self.faces_database = FacesDatabase(face_gallery, self.face_identifier,
                                            self.landmarks_detector,
                                            self.face_detector if args.run_detector else None, args.no_show)
    ...
    def process(self, frame):
        ...
        #print("Face detector:", delta*1000, 'ms')
...
class Visualizer:
    ...
    def draw_text_with_background(self, frame, text, origin,
                                  font=cv2.FONT_HERSHEY_COMPLEX, scale=1.5,
                                  color=(0, 0, 0), thickness=3, bgcolor=(255, 255, 255)):
...
##                                       font, scale=text_scale)
        color = (0, 0, 255)
        cv2.putText(frame, text, tuple(roi.position), font, 0.7, color, 2)
...
##                                       (origin + (0, text_size[1] * 1.5)), font, text_scale, color)
    ...
    def run(self, args):
        ...
        input_stream = Visualizer.open_input_stream(input_video)
        if input_stream is None or not input_stream.isOpened():
            #log.error("Cannot open input stream: %s" % args.input)
            log.error("Cannot open input stream: %s" % input_video)
